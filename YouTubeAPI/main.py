...
class YouTubeAPI(requests.Session):
	...
	def __init__(self, debug_level='ERROR'):
		...
		self._getInitialPlayerResponse = lambda html: (self._debug('INFO', 'Start parsing JSON data'), json.loads(self._findSnippet(html, 'var ytInitialPlayerResponse = ', '</script>', (0, 1))), self._debug('SUCCESS', 'JSON data parsed successfully'))[1]
...
class _Video(YouTubeAPI):
	def __init__(self, videoId, debug_level='ERROR'):
		...
		self._result = self._cleanupVideoData(self._player_data, self._init_data)
	...
	@_logException
	def _cleanupVideoData(self, player_data, init_data):
		primary_info = next(self._searchDict(init_data, "videoPrimaryInfoRenderer"))
		secondary_info = next(self._searchDict(init_data, "videoSecondaryInfoRenderer"))
		player_info = player_data["videoDetails"]
		return {
			'video_id': player_info["videoId"],
			'title': ''.join(i['text'] for i in primary_info['title']['runs']),
			'description': ''.join(i['text'] for i in secondary_info["description"]['runs']),
			'keywords': player_info["keywords"],
			'publish_time': primary_info["dateText"]["simpleText"],
			'views': int(player_info["viewCount"]),
			'author': {
			...
			"length_seconds": player_info["lengthSeconds"],
			'thumbnails': self._getThumbnail(player_info['videoId']),
			'sentiment_count': dict(zip(['like_count', 'unlike_count'], map(lambda i: int(i.replace(',', '')), primary_info["sentimentBar"]["sentimentBarRenderer"]["tooltip"].split(' / ')))),
			'itags': dict([(i['itag'], {
			...
			}) for i in player_data["streamingData"]["formats"]+player_data["streamingData"]["adaptiveFormats"]])
		}
	...
	@_logException
	def download(self, itag=None, path='.'):
		if itag and itag in self._result['itags'].keys():
			target = self._result['itags'][itag]
		else:
			target = list(self._result['itags'].values())[0]
		name = self._convertValidFilename(self._result['title'])
		extension = target['mime_type'].split(';')[0].split('/')[-1]
		open(os.path.join(path, f'{name}.{extension}'), 'wb').write(self.get(target['url']).content)
