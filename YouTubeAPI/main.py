...
class _Video(YouTubeAPI):
	...
	def _cleanupVideoData(self):
		json.dump(self._init_data, open('test2.json', 'w'), indent=4)
	...
	def _stream(self, url, chunk_size=4096):
		file_size: int = int(requests.get(url, stream=True).headers.get('content-length', 0))  # fake filesize to start
		downloaded = 0
	...
	def get_json(self, include_download_link=False):
		_result = self._cleanupVideoData()
		if not include_download_link: del _result['itags']
		return _result

	@_logException
	def download(self, itag=None, path=".", show_progress=True):
		_result = self.get_json(include_download_link=True)
		if itag and itag in _result["itags"].keys():
		...
		if show_progress: progress_bar = tqdm(total=int(requests.get(target['url'], stream=True).headers.get('content-length', 0)), unit='iB', unit_scale=True)
		with open(os.path.join(path, f"{name}.{extension}"), "wb") as f:
			for chunk in self._stream(target['url']):
				if show_progress: progress_bar.update(len(chunk))
				f.write(chunk)

if __name__ == "__main__":
	api = YouTubeAPI()
	#json.dump(api.search('the silly coder'), open('search.json', 'w'), indent=4)
	api.video("sS8DWRQ_tao").download()